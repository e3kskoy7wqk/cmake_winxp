...
static BOOL MakeLink( LPCWSTR src, LPCWSTR dst, DWORD  dwFlags)
{
...
error:
  ...
  return FALSE;
    ...
    pLinkInfo = (PFILE_LINK_INFORMATION)malloc( sizeof( *pLinkInfo) + uRel.Length);
    if ( NULL == pLinkInfo)
    {
        (m_pfnRtlFreeUnicodeString)( &uOldName);
        (m_pfnRtlFreeUnicodeString)( &uNewName);
                                  ...
                                  std::string const& newName)
{
  ...
  if (!MakeLink(newPath.c_str(), origPath.c_str(),
                           flags |
  ...
  if (status.GetWindows() == ERROR_INVALID_PARAMETER) {
    ...
    if (!MakeLink(newPath.c_str(), origPath.c_str(), flags)) {
      status = Status::Windows_GetLastError();
    }
  }
